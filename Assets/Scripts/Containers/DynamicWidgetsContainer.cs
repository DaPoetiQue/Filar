    ...
    public class DynamicWidgetsContainer : AppData.DynamicContainer
    {
        ...
        void UpdateAutoScrollWidget()
        {
            if (orientation == AppData.OrientationType.Vertical)
            {
                if (scroller.value.verticalNormalizedPosition == 1.0f)
                    ScreenUIManager.Instance.GetCurrentScreen().GetData().GetWidget(AppData.WidgetType.ScrollerNavigationWidget).GetData().SetActionButtonState(AppData.InputActionButtonType.ScrollToTopButton, AppData.InputUIState.Disabled);
                else
                    ScreenUIManager.Instance.GetCurrentScreen().GetData().GetWidget(AppData.WidgetType.ScrollerNavigationWidget).GetData().SetActionButtonState(AppData.InputActionButtonType.ScrollToTopButton, AppData.InputUIState.Enabled);

                if (scroller.value.verticalNormalizedPosition == 0.0f)
                    ScreenUIManager.Instance.GetCurrentScreen().GetData().GetWidget(AppData.WidgetType.ScrollerNavigationWidget).GetData().SetActionButtonState(AppData.InputActionButtonType.ScrollToBottomButton, AppData.InputUIState.Disabled);
                else
                    ScreenUIManager.Instance.GetCurrentScreen().GetData().GetWidget(AppData.WidgetType.ScrollerNavigationWidget).GetData().SetActionButtonState(AppData.InputActionButtonType.ScrollToBottomButton, AppData.InputUIState.Enabled);
            }
        ...
        protected override void OnInitialization()
        {
            ...
            scroller.Initialized(scrollerInitializedCallback =>
            {
                ...
                {
                    if (scroller.GetFadeUIScrollBar())
                        scroller.GetUIScrollBarComponent().SetVisibilityState(AppData.UIVisibilityState.Hidden);
                }
                else
