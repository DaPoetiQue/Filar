...
namespace Com.RedicalGames.Filar
{
    public class ScreenUIManager : AppData.SingletonBaseComponent<ScreenUIManager>
    {
            ...
            if (!screens.Contains(screen))
            {
                ...
                if (screens.Contains(screen))
                {
                    ...
                    if (callbackResults.Success())
                    {
                        ...
                        assetBundlesLibrary.GetDynamicContainer<DynamicScreenContainer>(AppData.ScreenType.Default, AppData.ContentContainerType.AppScreenContainer, AppData.ViewSpaceType.ScreenSpace, containerCallbackResults =>
                        {
        ...
        public async Task<AppData.CallbackData<Screen>> ShowScreenAsync(AppData.ScreenType screenType, AppData.ScreenBlurConfig blurConfig = null)
        {
            ...
            if (callbackResults.Success())
            {
                GetScreen(screenType, screenFoundCallbackResults =>
                {
                    callbackResults.SetResult(screenFoundCallbackResults);

                    if (callbackResults.Success())
                    {
                        var screen = screenFoundCallbackResults.GetData();
                        ...
                        if(callbackResults.Success())
                        {
                            ...
                            SetCurrentScreen(screen, currentScreenSetCallbackResults => 
                            {
                                callbackResults.SetResult(currentScreenSetCallbackResults);
        ...
        public async Task<AppData.CallbackData<Screen>> HideScreenAsync(AppData.ScreenType screenType, AppData.ScreenBlurConfig blurConfig = null)
        {
            ...
            if(callbackResults.Success())
            {
                callbackResults.SetResult(GetCurrentScreen());
        ...
        public async Task<AppData.CallbackData<Screen>> HideScreenAsync(AppData.ScreenConfigDataPacket dataPacket)
        {
            ...
            await OnHideSelectedScreenViewAsync(callbackResults.GetData());
            ...
            try
            {
                ...
                if (callbackResults.Success())
                {
                    if (screenType == GetCurrentScreenType().GetData())
                    {
                        ...
                        callbackResults.SetResult(AppData.Helpers.GetAppComponentsValid(posts, "Posts", "Failed to Get Loaded Posts. Please Check Here."));

                        if (callbackResults.Success())
                        {
                            callbackResults.SetResult(screenContentContainer.GetActive());

                            if (callbackResults.Success())
                            {
                                callbackResults.SetResult(databaseManager.GetAssetBundlesLibrary());

                                if (callbackResults.Success())
                                {
                                    var assetBundlesLibrary = appDatabaseManagerInstance.GetAssetBundlesLibrary().GetData();

                                    callbackResults.SetResult(assetBundlesLibrary.GetLoadedSelectableWidgets(screenType));

                                    if (callbackResults.Success())
                                    {
                                        var loadedSelectableWidgets = assetBundlesLibrary.GetLoadedSelectableWidgets(screenType).GetData();

                                        var postWidget = loadedSelectableWidgets.Find(widget => widget.GetType().GetData() == AppData.SelectableWidgetType.Post);

                                        callbackResults.SetResult(AppData.Helpers.GetAppComponentValid(postWidget, "Post Widget", "Failed To Find Selectable Widget Of Type : Post."));
